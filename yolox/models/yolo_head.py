...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        for i in range(len(in_channels)):             #三个尺度的分支，对应YOLO的FPN结果，然后每个分支都进行了解耦。
            self.stems.append(
                BaseConv(
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(                #cls_convs是列表，对应三个尺度分支的卷积预测。
        ...
        if self.training:
            ...
            self.hw = [x.shape[-2:] for x in outputs]     #取出每个特征图的宽高
            # [batch, n_anchors_all, 85]  按照第三维度压缩拼接
